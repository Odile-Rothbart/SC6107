...
- VRF Coordinator: **[PASTE HERE]**
- Subscription ID (subId): **[PASTE HERE]**
- KeyHash / gasLane: **[PASTE HERE]**
- callbackGasLimit: **[PASTE HERE]**
- requestConfirmations: **[PASTE HERE, if applicable]**

## B3. Acceptance Goals (Testnet)
...
## B4. Testnet Smoke Scenarios (Evidence-Based)
...
**Evidence (Explorer)**
- Dice request tx (user): **[PASTE TX HASH]**
- Dice VRF fulfill tx (Chainlink callback): **[PASTE TX HASH]**
- Optional post-state evidence:
  - Bet status/value/result: **[PASTE screenshot or brief note / event name if you use one]**
...
**Steps**
1. Open the dApp and connect wallet (Sepolia).![alt text](image.png)
2. Navigate to the Raffle page.
3. Enter the raffle by paying the entrance fee (confirm tx).
4. After the interval passes, trigger `performUpkeep` (via UI/admin action if exposed) OR wait for the configured mechanism to trigger it.
...
**Evidence (Explorer)**
- Raffle enter tx (user): **[PASTE TX HASH, optional but recommended]**
- Raffle upkeep/request tx (`performUpkeep`): **[PASTE TX HASH]**
- Raffle VRF fulfill tx (Chainlink callback): **[PASTE TX HASH]**
- Optional post-state evidence:
  - `recentWinner` / `raffleState`: **[PASTE screenshot or brief note]**

## B5. Notes / Limitations
